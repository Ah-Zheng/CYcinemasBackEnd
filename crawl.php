...
foreach ($movies as $index => $cm) {  //避免文字裡有'這個符號
    ...
    $movies[$index]['play_date'] = preg_replace("/\//", '-', substr($movies[$index]['play_date'], -10));
    $movies[$index]['actor'] = mb_substr($movies[$index]['actor'], 5);
    $movies[$index]['genre'] = mb_substr($movies[$index]['genre'], 5);
}
...
foreach ($coming_movies as $index => $cm) {
    ...
    $coming_movies[$index]['play_date'] = substr($coming_movies[$index]['play_date'], -10);
    $coming_movies[$index]['play_date'] = preg_replace("/\//", '-', substr($coming_movies[$index]['play_date'], -10));
    $coming_movies[$index]['actor'] = mb_substr($coming_movies[$index]['actor'], 5);
    $coming_movies[$index]['genre'] = mb_substr($coming_movies[$index]['genre'], 5);
}
